        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if names[int(cls)]=='person' or names[int(cls)]=='vehicle' or names[int(cls)]=='building' or names[int(cls)]=='tree' or names[int(cls)]=='bridge'or names[int(cls)]=='structure':  
                        if save_img or view_img:  # Add bbox to image
                            #label = '%s %.2f' % (names[int(cls)], conf)
                            #plot_one_box(xyxy, im0, label=label, color=colors[int(cls)], line_thickness=3)
                            label = '%s' % (names[int(cls)])
                            plot_one_box(xyxy, im0, label=label, color=colors, line_thickness=2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='runs/train/exp0/weights/best.pt', help='model.pt path(s)')
    parser.add_argument('--source', type=str, default='huinsdrone/images/test', help='source')  # file/folder, 0 for webcam
